...
static bool ui_remote_use_mqtt(void);
static const char *ui_remote_mode_name(void);
static uint16_t ui_remote_default_port(void);
static const char *ui_remote_saved_host(void);
static uint16_t ui_remote_saved_port(void);

static bool ui_remote_use_mqtt(void)
{
    return g_app_config.transport_mode == APP_TRANSPORT_MQTT;
}

static const char *ui_remote_mode_name(void)
{
    return ui_remote_use_mqtt() ? "MQTT" : "WebSocket";
}

static uint16_t ui_remote_default_port(void)
{
    return ui_remote_use_mqtt() ? 1883 : 5052;
}

static const char *ui_remote_saved_host(void)
{
    if (ui_remote_use_mqtt()) {
        return g_app_config.mqtt_host[0] ? g_app_config.mqtt_host : "192.168.100.1";
    }
    return g_app_config.ws_host[0] ? g_app_config.ws_host : "192.168.100.1";
}

static uint16_t ui_remote_saved_port(void)
{
    if (ui_remote_use_mqtt()) {
        return g_app_config.mqtt_port ? g_app_config.mqtt_port : ui_remote_default_port();
    }
    return g_app_config.ws_port ? g_app_config.ws_port : ui_remote_default_port();
}
...
static void start_btn_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        if (port == 0) {
            port = ui_remote_saved_port();
        }

        log_info("启动%s连接: %s:%d", ui_remote_mode_name(), host_text, port);

        /* 更新全局配置，并全量写回 ws_config.txt（保证所有配置项都在同一个文件里） */
        if (ui_remote_use_mqtt()) {
            strncpy(g_app_config.mqtt_host, host_text, sizeof(g_app_config.mqtt_host) - 1);
            g_app_config.mqtt_host[sizeof(g_app_config.mqtt_host) - 1] = '\0';
...
static void stop_btn_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        log_info("停止%s连接", ui_remote_mode_name());
...
{
    log_info("创建远程连接配置界面: mode=%s", ui_remote_mode_name());
    ...
    lv_label_set_text_fmt(title_label, "%s远程控制", ui_remote_mode_name());
    ...
    lv_label_set_text(g_ws_ui->device_id_label, remote_transport_get_device_id());
    ...
    lv_label_set_text(host_label, ui_remote_use_mqtt() ? "Broker地址:" : "服务器地址:");
    ...
    const char *saved_host = ui_remote_saved_host();
    uint16_t saved_port = ui_remote_saved_port();
    ...
    log_info("%s配置界面创建成功", ui_remote_mode_name());
...
void ui_websocket_update_status(bool connected, const char *host, uint16_t port)
{
    ...
    if (connected) {
        snprintf(status_text, sizeof(status_text),
                "状态: %s已连接到 %s:%d", ui_remote_mode_name(), host, port);
        ...
        snprintf(status_text, sizeof(status_text),
                "状态: %s已断开 (上次连接: %s:%d)", ui_remote_mode_name(), host, port);
        lv_label_set_text(g_ws_ui->status_label, status_text);
